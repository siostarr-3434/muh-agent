...
test('server auth uses password login and no magic-link route', async () => {
  ...
  assert.match(api, /\/api\/knowledge/)
  assert.match(api, /\/api\/calendar\/connect/)
  assert.match(api, /\/api\/calendar\/sync/)
  assert.match(api, /invoke\('gmail-oauth-start'/)
  assert.match(api, /invoke\('calendar-sync'/)
